...
interface SavingsProps {
  ...
  setBalance: React.Dispatch<React.SetStateAction<number>>; // ✅ Pastikan saldo utama dikurangi saat menabung
  balance: number;
}

const Savings: React.FC<SavingsProps> = ({ setSavings, setBalance, balance }) => {
      ...
      <Box sx={{ maxWidth: 600, width: "100%" }}>
        <Card sx={{ padding: 3, boxShadow: "0px 4px 10px rgba(0,0,0,0.1)" }}>
          <CardContent>
            ...
            <TextField label="Nama Tabungan" value={name} onChange={(e) => setName(e.target.value)} fullWidth margin="normal" />
            <TextField
              label="Jumlah"
              type="number"
              value={amount === "" ? "" : amount} // ✅ Kosongkan jika 0
              onChange={(e) => {
                const newValue = e.target.value;
                if (/^\d*$/.test(newValue)) {
                  setAmount(newValue === "" ? "" : Number(newValue)); // ✅ Pastikan angka tidak dimulai dengan 0
                }
              }}
              fullWidth
              margin="normal"
            ...
            <List sx={{ marginTop: 3 }}>
              {savingsList.length === 0 ? (
                <Typography textAlign="center" sx={{ color: "gray" }}>Belum ada tabungan</Typography>
              ) : (
                savingsList.map((saving) => (
                  <ListItem key={saving.id} sx={{ borderBottom: "1px solid #eee" }}>
                    <ListItemText primary={saving.name} secondary={`Rp ${saving.amount.toLocaleString()}`} />
                  </ListItem>
                ))
              )}
            </List>
